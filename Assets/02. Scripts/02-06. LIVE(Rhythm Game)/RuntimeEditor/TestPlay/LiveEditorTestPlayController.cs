...
///
/// 테스트 플레이는 곡을 처음부터 새로 트는 별도의 모드가 아니라, 편집 상태에서 "노트 편집만 막고 입력과 판정을 켠"
/// 상태입니다. 지금 보고 있는 재생 지점에서 그대로 이어 쳐 보고, 멈추면 멈춘 그 자리에서 곧바로 채보를 고칠 수 있어야
/// 배치가 적절한지 반복해서 확인할 수 있기 때문입니다. 그래서 진입·종료 어느 쪽에서도 재생 위치를 옮기지 않습니다.
///
/// 판정·집계·입력은 리듬게임 씬과 완전히 같은 컴포넌트를 그대로 씁니다.
/// 에디터 트랙(LiveEditorTimeline)이 LiveTrackScroller를 상속하고 있어, 판정 처리기가 판정된 노트를 가리는
/// 경로까지 손대지 않고 맞물립니다.
///
/// LiveConductor는 dspTime을 기준으로 재생 시각을 보간하므로 배속 1.0을 전제로 합니다.
/// 배속이 걸린 채로 테스트하면 판정 시각과 실제 소리가 계속 어긋나므로 테스트 중에는 배속을 잠급니다.
/// </summary>
