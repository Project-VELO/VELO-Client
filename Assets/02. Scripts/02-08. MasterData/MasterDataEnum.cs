...
///
/// JsonUtility는 열거형을 선언 순서에 따른 정수로만 직렬화합니다.
/// 도구가 생성하는 파일이라면 상관없지만 마스터 데이터는 사람이 직접 편집하므로,
/// "MinimumRank": 3 처럼 숫자를 적게 하면 의미를 알 수 없고 오타가 조용히 통과합니다.
/// 특히 ELiveRank는 좋은 랭크일수록 값이 작아 숫자로 적을 때 착각하기 쉽습니다.
///
/// 그래서 마스터 데이터 DTO는 열거형을 문자열 필드로 받아 두고, 역직렬화 직후 이 헬퍼로 변환합니다.
/// 세이브 데이터는 사람이 편집하지 않으므로 기존대로 정수 직렬화를 유지합니다.
/// </summary>
