...
///
/// 호출부가 홈(일반 LIVE·스케줄 바로가기), 스페이스(연습실), 사무실(스케줄 바로가기)에 걸쳐 네 곳 생깁니다.
/// 화면마다 SetEntry와 씬 이동을 따로 쓰면 한 곳만 빠뜨려도 이전 진입 타입이 그대로 남습니다.
/// 예를 들어 연습실에서 나온 뒤 홈 LIVE를 시작하면 PRACTICE_LIVE가 잔류해, 스케줄이 완료되지 않고
/// 보상도 5%만 지급됩니다. 두 동작을 붙여 두어 누락이 구조적으로 불가능하게 만듭니다.
///
/// GameProgressService가 상태 변경과 저장을 묶어 둔 것과 같은 이유입니다.
/// </summary>
public static class LiveEntryStarter
{
    ...
    ///
    /// designatedSongId를 넘기면 그 곡으로 고정되고 목록에서 다른 곡을 고를 수 없습니다(기획서.txt:490-495).
    /// 자유 선택으로 진입할 때는 두 인자를 비워 둡니다(기획서.txt:482-488).
    ///
    /// 홈의 일일 스케줄 바로가기도 HOME_LIVE입니다(기획서.txt:440). 이름만 보고 SCHEDULE_LIVE를 넣으면
