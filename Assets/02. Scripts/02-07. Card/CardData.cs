...
/// 멤버 슬롯에는 그 멤버의 카드만 배치할 수 있으므로(기획서 3-H-2) 슬롯 정보를 카드에 중복해 두지 않습니다.
///
/// 필드가 public인 것은 의도적입니다. 사람이 직접 편집하는 JSON이라
/// 밑줄 접두사가 키에 노출되면 곤란합니다(Convention 1-b-(2)).
/// </summary>
[Serializable]
public class CardData
{
    ...
    /// <summary>
    /// 카드 등급(★)입니다. 아직 표시에만 사용하며 점수에 반영하지 않습니다.
    /// </summary>
    public int Grade = 1;

    /// <summary>
    /// 카드 능력치입니다. 아직 점수·판정에 반영하지 않으나(기획서 3-H-1) 스키마는 미리 둡니다.
    /// </summary>
    public CardStats Stats = new CardStats();

    /// <summary>
    /// 카드 고유 스킬입니다. 아직 쓰이지 않습니다.
    /// </summary>
