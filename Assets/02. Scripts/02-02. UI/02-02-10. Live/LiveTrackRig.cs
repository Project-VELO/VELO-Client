...
///
/// 2D 트랙은 사다리꼴을 직접 그렸지만, 3D에서는 트랙이 실제로는 폭이 일정한 평평한 띠이고
/// 사다리꼴 모양은 카메라의 원근 투영이 만들어 냅니다. 따라서 노트는 월드 공간에서 등속으로 다가오기만 하면 되고,
/// 화면상의 가속과 확대는 전부 카메라가 처리합니다.
///
/// 카메라 파라미터는 하드코딩하지 않고 디자인 수치(판정선 폭/높이, 트랙 상단 폭)에서 역산하므로,
/// 어떤 화각을 골라도 화면에 맺히는 트랙 모양은 2D 씬과 완전히 같습니다.
/// </summary>
[ExecuteAlways]
